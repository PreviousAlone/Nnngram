...
//        TLRPC.StickerSet stickerSet = messages_stickerSet.set;
    ...
    public ArrayList<TLRPC.MessageEntity> getEntities(CharSequence[] message, boolean allowStrike, boolean parseMarkdown) {
        ...
        final String mono = "`";
        final String pre = "```";
        while (!(Config.newMarkdownParser || EditTextBoldCursor.disableMarkdown) && parseMarkdown && (index = TextUtils.indexOf(message[0], !isPre ? mono : pre, lastIndex)) != -1) {
            if (start == -1) {
        ...
        if (!EditTextBoldCursor.disableMarkdown && Config.newMarkdownParser && parseMarkdown) EntitiesHelper.parseMarkdown(message, allowStrike);
