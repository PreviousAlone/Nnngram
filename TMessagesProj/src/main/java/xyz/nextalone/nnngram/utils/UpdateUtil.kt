    ...
    fun postCheckFollowChannel(ctx: Context, currentAccount: Int) =
        UIUtil.runOnIoDispatcher {
            ...
            if (ConfigManager.getBooleanOrFalse(
                    Defines.updateChannelSkip
                )
            ) return@runOnIoDispatcher

            val messagesCollector =
            ...
            val updateChannel =
                messagesCollector.getUserOrChat(
                    channelUsername
                )
            ...
            if (updateChannel is TLRPC.Chat) checkFollowChannel(
                ...
                connectionsManager.sendRequest(
                    ...
                    if (error == null) {
                        ...
                        val chat =
                            res.chats.find { it.username == channelUsername }
                                ?: return@sendRequest
                        messagesCollector.putChats(
                            res.chats,
                            false
                        )
                        messagesStorage.putUsersAndChats(
                            res.users,
                            res.chats,
                            false,
                            true
